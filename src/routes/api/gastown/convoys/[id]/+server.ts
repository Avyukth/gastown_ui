...
export const GET: RequestHandler = async ({ params }) => {
	...
	if (!id || !/^[a-zA-Z0-9_-]+$/.test(id)) {
		return json({ error: 'Invalid convoy ID', requestId }, { status: 400 });
	}
	...
	try {
		...
		if (!result.success) {
			...
			if (errorMessage.includes('no issue found') || errorMessage.includes('not found')) {
				return json({ error: 'Convoy not found', requestId }, { status: 404 });
			}
			console.error(`Failed to fetch convoy ${id}:`, result.error);
			return json(
				{
		...
		if (bead.issue_type !== 'convoy') {
			return json({ error: 'Not a convoy', requestId }, { status: 400 });
		}
